...
declare -a CELL_SUPERRES_FOLDERS=(
    ...
    '/scratch/sycamore-asap/ASAP_Members_Other_Imaging_Data/JSB/20251026_MassiveCells/Ximea/250pMATTO655_750pMCy3B_20perc561_60mW638_1/'
...
# Process SM data hierarchical directories first (highest priority)
console_message "Processing SM data directories first (${#SM_DATA_DIRS[@]} base directories)..."
...
# Process cell super-resolution folders (647nm wavelength)
console_message "Processing cell super-resolution folders (${#CELL_SUPERRES_FOLDERS[@]} folders)..."
for folder in "${CELL_SUPERRES_FOLDERS[@]}"; do
    if [ -d "$folder" ]; then
        process_folder "imaging" "$folder" "0.647"
    else
        log_message "WARNING: Cell super-resolution folder not found: $folder"
    fi
done
