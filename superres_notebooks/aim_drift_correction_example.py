...
def load_example_data() -> Tuple[np.recarray, list]:
    ...
    true_drift_x = 2.0 * np.sin(frames / 100) + 0.01 * frames  # 2 pixel sine wave + linear
    true_drift_y = 1.5 * np.cos(frames / 80) + 0.005 * frames   # 1.5 pixel cosine + linear
    ...
    for frame in range(n_frames):
        ...
        base_x = np.random.choice([20, 40, 60, 80], n_locs_per_frame) + np.random.normal(0, 2, n_locs_per_frame)
        base_y = np.random.choice([15, 35, 55, 75], n_locs_per_frame) + np.random.normal(0, 2, n_locs_per_frame)
        ...
        final_x = drifted_x + np.random.normal(0, 0.02, n_locs_per_frame)  # 20nm precision
        final_y = drifted_y + np.random.normal(0, 0.02, n_locs_per_frame)  # 20nm precision
    ...
    print(f"✅ Created example data: {len(locs)} localizations across {n_frames} frames")
    print(f"   True drift range: X = {true_drift_x.min():.2f} to {true_drift_x.max():.2f} pixels")
    print(f"                     Y = {true_drift_y.min():.2f} to {true_drift_y.max():.2f} pixels")
...
def analyze_correction_quality(
    ...
    metrics = {
        ...
        "drift_magnitude": np.sqrt(drift_result.drift_x**2 + drift_result.drift_y**2).max()
    }
...
def plot_before_after_comparison(
    ...
    plt.tight_layout()
    plt.savefig('/tmp/aim_drift_correction_comparison.png', dpi=150, bbox_inches='tight')
