...
class InteractiveThresholdTuner:
    ...
    def test_spot_detection(self, image: np.ndarray, pfa: float, 
                          perc_threshold: float, wavelength: float, 
                          read_noise: np.ndarray = None, 
                          hot_pixel_threshold: float = 20.0) -> Tuple[np.ndarray, int]:
        """Test spot detection with given parameters including hot pixel filtering"""
        try:
    ...
    def interactive_parameter_tuning(self, folder_path: str, folder_type: str, 
        ...
        current_hot_pixel_threshold = 20.0  # Default hot pixel threshold
        ...
        while True:
            ...
            spots, num_spots = self.test_spot_detection(
                ...
                hot_pixel_threshold=current_hot_pixel_threshold)
            ...
            print(f"  Hot pixel threshold: {current_hot_pixel_threshold}")
            ...
            print(f"  4. Adjust hot pixel threshold (current: {current_hot_pixel_threshold})")
            ...
            if choice == '1':
                ...
                try:
                    new_hot_pixel_threshold = float(input(f"Enter new hot pixel threshold (current: {current_hot_pixel_threshold}): ").strip())
                    if new_hot_pixel_threshold > 0:
                        current_hot_pixel_threshold = new_hot_pixel_threshold
                    else:
                        print("Hot pixel threshold must be positive, keeping current value")
                except ValueError:
                    print("Invalid input, keeping current value")
    ...
    def save_threshold_parameters(self, output_file: str = 'threshold_parameters.txt'):
        ...
        with open(output_path, 'w') as f:
            ...
            f.write("# Format: folder_path|pfa|perc_threshold|wavelength|hot_pixel_threshold\n")
            f.write("#\n")
            
            for folder_path, params in self.threshold_results.items():
                hot_pixel_thresh = params.get('hot_pixel_threshold', 20.0)  # Default if not present
                f.write(f"{folder_path}|{params['pfa']:.0e}|{params['perc_threshold']:.1f}|{params['wavelength']:.3f}|{hot_pixel_thresh:.1f}\n")
