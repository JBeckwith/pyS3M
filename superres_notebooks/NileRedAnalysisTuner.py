...
Usage:
    ...
    Arguments:
        folder_path: Path to the folder containing Nile Red imaging data

Output:
    nile_red_threshold_parameters.txt - Parameter file for NileRedAnalysis.sh
...
class NileRedThresholdTuner:
    ...
    def __init__(self, folder_path: str):
        ...
        # Folder path to process (user-provided)
    ...
    def get_all_processing_folders(self) -> List[Tuple[str, str, float]]:
        """Get all folders that will be processed with their parameters"""
        all_folders = []

        # Process the user-provided folder path
        if os.path.isdir(self.folder_path):
            # Check if this is a leaf directory or has subdirectories
            leaf_dirs = self.find_leaf_directories(self.folder_path)
            if leaf_dirs:
                ...
                all_folders.append((self.folder_path, "imaging", 0.650))
    ...
    def save_threshold_parameters(self, output_file: str = "nile_red_threshold_parameters.txt"):
    ...
    def run(self):
        ...
        print("=" * 80)
        print("Nile Red Interactive Threshold Tuner")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        print("\nUsage:")
        print("  python NileRedAnalysisTuner.py <folder_path>")
        print("\nExample:")
        print("  python NileRedAnalysisTuner.py /scratch/sycamore-asap/ASAP_Members_Other_Imaging_Data/JSB/20251128_SAureus/data")
        sys.exit(1)
    ...
    tuner = NileRedThresholdTuner(folder_path)
    tuner.run()