...
def create_example_data() -> Tuple[np.recarray, List[dict]]:
    ...
    print(f"   - True drift range: X=[{true_drift_x.min():.3f}, {true_drift_x.max():.3f}] pixels")
    print(f"                       Y=[{true_drift_y.min():.3f}, {true_drift_y.max():.3f}] pixels")
...
def step1_detect_fiducials(
    ...
    print(f"   Total candidates detected: {detection_result.metadata['total_candidates']}")
    print(f"   Localizations per fiducial: {detection_result.metadata['localizations_per_fiducial']}")
...
def step2_drift_correction(
    ...
    print(f"Using {detection_result.n_fiducials} detected fiducials for drift correction...")
    ...
    print(f"   Maximum drift magnitude: {np.sqrt(drift_result.drift_x**2 + drift_result.drift_y**2).max():.3f} pixels")
...
def analyze_correction_quality(
    ...
    fiducial_locs = corrected_locs[corrected_locs.group >= 0] if hasattr(corrected_locs, 'group') else []
    ...
    if len(fiducial_locs) > 0:
        ...
        for group_id in unique_groups:
            ...
            print(f"{group_id+1:<3} {len(group_locs):<6} ({center_x:.2f},{center_y:.2f})   ({precision_x:.0f},{precision_y:.0f})")
    ...
    metrics = {
        ...
        'fiducial_precisions': fiducial_precisions if 'fiducial_precisions' in locals() else [],
        'detection_method': 'separated_workflow'
    }
    
    print(f"\\nOverall Quality Summary:")
    print(f"  - Fiducials used: {metrics['n_fiducials']}")
    print(f"  - Coordinate spread before: {orig_spread_x:.0f} nm (X), {orig_spread_y:.0f} nm (Y)")
    print(f"  - Coordinate spread after: {corr_spread_x:.0f} nm (X), {corr_spread_y:.0f} nm (Y)")
    print(f"  - Improvement factors: {improvement_x:.2f}x (X), {improvement_y:.2f}x (Y)")
...
def create_comprehensive_plots(
    ...
    ax = axes[0]
    ax.scatter(original_locs.xc*1000, original_locs.yc*1000, s=1, alpha=0.3, c='red')
    
    # Add fiducial markers
    for i, (x, y) in enumerate(detection_result.picks):
        ax.plot(x * detection_result.detection_params['pixelsize'], 
               y * detection_result.detection_params['pixelsize'], 
               'co', markersize=8, markerfacecolor='none', markeredgewidth=2)
        ax.text(x * detection_result.detection_params['pixelsize'], 
               y * detection_result.detection_params['pixelsize'] + 500, 
               f'F{i+1}', color='cyan', ha='center', fontweight='bold')
    ...
    ax = axes[1]
    ax.scatter(corrected_locs.xc*1000, corrected_locs.yc*1000, s=1, alpha=0.3, c='blue')
    ...
    if hasattr(detection_result.locs_with_groups, 'group'):
        fiducial_locs = detection_result.locs_with_groups[detection_result.locs_with_groups.group >= 0]
        if len(fiducial_locs) > 0:
            unique_groups = np.unique(fiducial_locs.group)
            ...
            for i, group_id in enumerate(unique_groups):
                group_locs = fiducial_locs[fiducial_locs.group == group_id]
                ax.scatter(group_locs.xc*1000, group_locs.yc*1000, s=3, alpha=0.6,
                          c=[colors[i]], label=f'F{group_id+1}')
    ...
    # Overall title
    fig.suptitle('Separated Fiducial Drift Correction Workflow Results', 
...
def save_results(
    ...
    summary_data = {
        ...
        'drift_summary': {
            ...
            'max_magnitude': float(np.sqrt(drift_result.drift_x**2 + drift_result.drift_y**2).max())
        }
    }
...
def main():
    ...
    print(f"✅ Max drift corrected: {quality_metrics['max_drift_magnitude']:.3f} pixels")
    print(f"✅ Improvement factors: {quality_metrics['improvement_factor_x']:.2f}x (X), {quality_metrics['improvement_factor_y']:.2f}x (Y)")
