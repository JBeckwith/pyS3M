...
try:
    ...
    warnings.warn("Could not import render/imageprocess modules. RCC method may not work.")
    _render = None
    _imageprocess = None
...
class CoordinateProcessor:
    ...
    @staticmethod 
    def apply_drift_correction(locs: np.recarray, drift_result: DriftResult) -> np.recarray:
...
class RCCDriftCorrector(DriftCorrector):
    ...
    def _generate_segments(
        ...
        bounds = np.linspace(
            0, meta["n_frames"] - 1, n_segments + 1, dtype=np.uint32
        )
...
class AIMDriftCorrector(DriftCorrector):
    ...
    def _run_aim_2d(
        ...
        x_pdc, y_pdc, drift_x1, drift_y1 = self._intersection_max(
            locs.xc, locs.yc, ref_x, ref_y,
            frame, seg_bounds, params.intersect_d, params.roi_r, width,
            aim_round=1, progress_callback=params.progress_callback
        )
        ...
        x_pdc, y_pdc, drift_x2, drift_y2 = self._intersection_max(
            ...
            frame, seg_bounds, params.intersect_d, params.roi_r, width,
            aim_round=2, progress_callback=params.progress_callback
        )
    ...
    def _run_aim_3d(
        ...
        z_pdc, drift_z1 = self._intersection_max_z(
            x_pdc, y_pdc, z, ref_x, ref_y, ref_z,
            frame, seg_bounds, params.intersect_d, params.roi_r, 
...
def undrift_rcc(
    ...
    params = DriftParameters(
        ...
        display=display,
        progress_callback=rcc_callback
    )
...
class Drift_Correction_Functions:
    ...
    def method_info(self, method: Union[str, DriftMethod]) -> Dict[str, Any]:
        ...
        return {
            ...
            "description": corrector.__class__.__doc__.split('\n')[0] if corrector.__class__.__doc__ else ""
        }