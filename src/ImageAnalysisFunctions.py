...
class FittingParameters:
    ...
    def _validate_parameters(self) -> None:
        ...
        if not all(len(arr) == n_puncta for arr in [
            self.smoothed_puncta, self.weights, self.relative_coords, self.planes
        ]):
        ...
        colour_strategies = {FittingStrategy.STANDARD, FittingStrategy.JUSTCOLOUR, 
        ...
        if self.masks is not None and len(self.masks) != n_puncta:
            raise FittingValidationError(
                "Masks array must have same length as puncta"
            )
...
class JustColourFittingProcessor(FittingProcessor):
    ...
    def _generate_initial_guess(
        ...
        return np.array([
            center[1], center[0],  # x, y
            1.0, 1.0,  # sigmas
            max_val * 0.4, max_val * 0.6,  # A_G, A_R ratios
            np.min(smoothed_punctum), np.min(smoothed_punctum),  # backgrounds
            0.0, 0.0  # theta, offset
        ])
...
class RawColourFittingProcessor(FittingProcessor):
    ...
    def _generate_initial_guess(
        ...
        return np.array([
            center[1], center[0],  # x, y
            1.0, 1.0,  # sigmas
            max_val * 0.3, max_val * 0.4, max_val * 0.3,  # Raw colour amplitudes
            np.min(smoothed_punctum),  # background
            0.0, 0.0  # theta, offset
        ])
...
class PosthenColourFittingProcessor(FittingProcessor):
    ...
    def _generate_initial_guess(
        ...
        return np.array([
            center[1], center[0],  # x, y
            1.0, 1.0,  # sigmas
            max_val * 0.4, max_val * 0.6,  # Enhanced colour ratios
            np.min(smoothed_punctum), np.min(smoothed_punctum),  # backgrounds
            0.0, 0.0  # theta, offset
        ])
    ...
    def _perform_posthencolour_fit(
        ...
        try:
            ...
            pfit_pos_leastsq, perr_pos_leastsq = nocolour_processor._perform_nocolour_fit(
                grayscale_punctum,
            ...
            locparams = np.array([
                pfit_pos_leastsq[0], pfit_pos_leastsq[1], pfit_pos_leastsq[2], 
                pfit_pos_leastsq[3], pfit_pos_leastsq[4], pfit_pos_leastsq[5]
            ])
            ...
            chisqr_colour = np.sum(
                np.square(
                    gaussoptfuncs.WLS_rawcolour_chi_nobounds(
                        pfit_colour, raw_punctum, masks, weights, size, ravelsize, locparams
                    )
                )
            ) / (len(raw_punctum.ravel()) - len(initial_guess_colour))
            
            # Process covariance matrix
            if (len(raw_punctum.ravel()) > len(initial_guess_colour)) and pcov_colour is not None:
                s_sq = chisqr_colour
            ...
            pfit_leastsq = np.concatenate([pfit_pos_leastsq[:-1], pfit_colour])  # Exclude position chi-squared
            perr_leastsq = np.concatenate([perr_pos_leastsq[:-1], perr_colour_leastsq])  # Exclude position error
...
class Image_Analysis_Functions:
    ...
    # Backward compatibility methods - delegate to new unified methods
    def fit_puncta(self, puncta, smoothed_puncta, masks, weights, relative_coords, planes):
        """Fit puncta using standard colour strategy (backward compatibility)."""
        return self.fit_puncta_method(
            puncta, smoothed_puncta, weights, relative_coords, planes,
            FittingStrategy.STANDARD, masks
        )
    
    def fit_nocolour_puncta(self, puncta, smoothed_puncta, weights, relative_coords, planes):
        """Fit puncta using no-colour strategy (backward compatibility)."""
        return self.fit_puncta_method(
            puncta, smoothed_puncta, weights, relative_coords, planes,
            FittingStrategy.NOCOLOUR
        )
    
    def fit_justcolour_puncta(self, puncta, smoothed_puncta, masks, weights, relative_coords, planes):
        """Fit puncta using just-colour strategy (backward compatibility)."""
        return self.fit_puncta_method(
            puncta, smoothed_puncta, weights, relative_coords, planes,
            FittingStrategy.JUSTCOLOUR, masks
        )
    
    def fit_rawcolour_puncta(self, puncta, smoothed_puncta, masks, weights, relative_coords, planes):
        """Fit puncta using raw-colour strategy (backward compatibility)."""
        return self.fit_puncta_method(
            puncta, smoothed_puncta, weights, relative_coords, planes,
            FittingStrategy.RAWCOLOUR, masks
        )
    
    def fit_posthencolour_puncta(self, puncta, smoothed_puncta, masks, weights, relative_coords, planes):
        """Fit puncta using post-colour strategy (backward compatibility)."""
        return self.fit_puncta_method(
            puncta, smoothed_puncta, weights, relative_coords, planes,
            FittingStrategy.POSTHENCOLOUR, masks
        )
    
    # Parallel compatibility methods
    def fit_puncta_parallel(self, puncta, smoothed_puncta, masks, weights, relative_coords, planes, asynch=False):
        """Parallel fit puncta using standard strategy (backward compatibility)."""
        return self.fit_puncta_parallel_method(
            puncta, smoothed_puncta, weights, relative_coords, planes,
            FittingStrategy.STANDARD, masks, asynch
        )
    
    def fit_nocolour_puncta_parallel(self, puncta, smoothed_puncta, weights, relative_coords, planes, asynch=False):
        """Parallel fit puncta using no-colour strategy (backward compatibility)."""
        return self.fit_puncta_parallel_method(
            puncta, smoothed_puncta, weights, relative_coords, planes,
            FittingStrategy.NOCOLOUR, None, asynch
        )
    
    def fit_justcolour_puncta_parallel(self, puncta, smoothed_puncta, masks, weights, relative_coords, planes, asynch=False):
        """Parallel fit puncta using just-colour strategy (backward compatibility)."""
        return self.fit_puncta_parallel_method(
            puncta, smoothed_puncta, weights, relative_coords, planes,
            FittingStrategy.JUSTCOLOUR, masks, asynch
        )
    
    def fit_rawcolour_puncta_parallel(self, puncta, smoothed_puncta, masks, weights, relative_coords, planes, asynch=False):
        """Parallel fit puncta using raw-colour strategy (backward compatibility)."""
        return self.fit_puncta_parallel_method(
            puncta, smoothed_puncta, weights, relative_coords, planes,
            FittingStrategy.RAWCOLOUR, masks, asynch
        )
    
    def fit_posthencolour_puncta_parallel(self, puncta, smoothed_puncta, masks, weights, relative_coords, planes, asynch=False):
        """Parallel fit puncta using post-colour strategy (backward compatibility)."""
        return self.fit_puncta_parallel_method(
            puncta, smoothed_puncta, weights, relative_coords, planes,
            FittingStrategy.POSTHENCOLOUR, masks, asynch
        )
    
    # Single punctum compatibility methods
    def fit_punctum(self, punctum, smoothed_punctum, masks, weights, relative_coords):
        """Fit single punctum using standard strategy (backward compatibility)."""
        processor = self.processors[FittingStrategy.STANDARD]
        return processor.fit_single_punctum(
            punctum, smoothed_punctum, weights, relative_coords, masks
        )
    
    def fit_nocolour_punctum(self, punctum, smoothed_punctum, weights, relative_coords):
        """Fit single punctum using no-colour strategy (backward compatibility)."""
        processor = self.processors[FittingStrategy.NOCOLOUR]
        return processor.fit_single_punctum(
            punctum, smoothed_punctum, weights, relative_coords
        )
    
    def fit_justcolour_punctum(self, punctum, smoothed_punctum, masks, weights, relative_coords):
        """Fit single punctum using just-colour strategy (backward compatibility)."""
        processor = self.processors[FittingStrategy.JUSTCOLOUR]
        return processor.fit_single_punctum(
            punctum, smoothed_punctum, weights, relative_coords, masks
        )
    
    def fit_rawcolour_punctum(self, punctum, smoothed_punctum, masks, weights, relative_coords):
        """Fit single punctum using raw-colour strategy (backward compatibility)."""
        processor = self.processors[FittingStrategy.RAWCOLOUR]
        return processor.fit_single_punctum(
            punctum, smoothed_punctum, weights, relative_coords, masks
        )
    
    def fit_posthencolour_punctum(self, punctum, smoothed_punctum, masks, weights, relative_coords):
        """Fit single punctum using post-colour strategy (backward compatibility)."""
        processor = self.processors[FittingStrategy.POSTHENCOLOUR]
        return processor.fit_single_punctum(
            punctum, smoothed_punctum, weights, relative_coords, masks
        )

    # WLS fitting compatibility methods
    def WLS_fit_colour_nobounds(self, data, initial_guess, masks, weights, relative_coords, display=False):
        """WLS colour fitting (backward compatibility)."""
        processor = self.processors[FittingStrategy.STANDARD]
        return processor._perform_wls_fit(data, initial_guess, masks, weights, relative_coords)
    
    def WLS_fit_nocolour_nobounds(self, data, initial_guess, weights, relative_coords, display=False):
        """WLS no-colour fitting (backward compatibility)."""
        processor = self.processors[FittingStrategy.NOCOLOUR]
        return processor._perform_nocolour_fit(data, initial_guess, weights, relative_coords)
    
    def WLS_fit_justcolour_nobounds(self, data, initial_guess, masks, weights, relative_coords, display=False):
        """WLS just-colour fitting (backward compatibility)."""
        processor = self.processors[FittingStrategy.JUSTCOLOUR]
        return processor._perform_justcolour_fit(data, initial_guess, masks, weights, relative_coords)
    
    def WLS_fit_rawcolour_nobounds(self, data, initial_guess, masks, weights, relative_coords, display=False):
        """WLS raw-colour fitting (backward compatibility)."""
        processor = self.processors[FittingStrategy.RAWCOLOUR]
        return processor._perform_rawcolour_fit(data, initial_guess, masks, weights, relative_coords)

    def gaussian_model(self, x, y, sigma_x, sigma_y, amplitudes, x_0, y_0, offset, theta, maskR, maskG, maskB):
        """Gaussian model for fitting (backward compatibility).
        
        This method maintains the original gaussian_model interface for
        backward compatibility with existing code.
        """
        # This would implement the actual Gaussian model calculation
        # For now, placeholder to maintain interface compatibility
        return np.zeros_like(x)  # Placeholder implementation