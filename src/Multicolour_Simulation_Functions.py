...
class MultiC_Sim_Funcs:
    ...
    def _colour_fit_averager(self, fit_results, n_bootstrap):
        """
    ...
    def test_demosaic_IG_fit_method(
        self,
        dye,
        filters,
        wavelength,
        ...
        for i, n_photon in enumerate(n_photon_space):
            ...
            error_grayscale = deepcopy(grayscale_smoothed_data)
            error_grayscale[error_grayscale < 0] = 0
            error_grayscale = error_grayscale + 1
            error_grayscale_map = np.add(error_grayscale, np.square(camera_parameters["readnoise"]))
            ...
            for frame in np.arange(n_bootstrap):
                ...
                locparams.append((fit_results['xc'][frame], fit_results['yc'][frame], fit_results['s_x'][frame], fit_results['s_y'][frame]))
            ...
            fit_results_colour = pd.DataFrame(fit_results_colour, columns=['bg_B', 'bg_G', 'bg_R', 'A_B', 'A_G', 'A_R', 'chi_sqr', 'frame']).sort_values(by=["frame"])
            fit_results_colour['photons'] = fit_results_colour['A_B'] + fit_results_colour['A_G'] + fit_results_colour['A_R']
            fit_results_colour['background_photons'] = fit_results_colour['bg_B'] + fit_results_colour['bg_G'] + fit_results_colour['bg_R']
            fit_results_colour['A_B'] = fit_results_colour['A_B'] / fit_results_colour['photons']
            fit_results_colour['A_G'] = fit_results_colour['A_G'] / fit_results_colour['photons']
            fit_results_colour['A_R'] = fit_results_colour['A_R'] / fit_results_colour['photons']
            fit_results_colour['bg_B'] = fit_results_colour['bg_B'] / fit_results_colour['background_photons']
            fit_results_colour['bg_G'] = fit_results_colour['bg_G'] / fit_results_colour['background_photons']
            fit_results_colour['bg_R'] = fit_results_colour['bg_R'] / fit_results_colour['background_photons']

            fit_results = pd.concat([fit_results, fit_results_colour], axis=1)
    ...
    def test_demosaic_fast_fit_method(
        self,
        dye,
        filters,
        wavelength,
        ...
        for i, n_photon in enumerate(n_photon_space):
            ...
            photoelectron_data, grayscale_photoelectron_data = sCMOS.bayer_demosaic_stack(photoelectron_data, True)
            ...
            smoothed_data, grayscale_smoothed_data = sCMOS.bayer_demosaic_stack(smoothed_data, True)
            ...
            for frame in np.arange(n_bootstrap * 3):
                ...
                weights_tofit.append(weights_map[frame, :, :])
                idx = frame // 3
                locparams.append((fit_results['xc'][idx], fit_results['yc'][idx], fit_results['s_x'][idx], fit_results['s_y'][idx]))
                planes.append(frame)
            ...
            fit_results_colour = pd.DataFrame(fit_results_colour, columns=['A', 'b', 'chi_sqr', 'frame']).sort_values(by=["frame"])
            fit_results_colour = self._colour_fit_averager(fit_results_colour, n_bootstrap)
            fit_results = pd.concat([fit_results, fit_results_colour], axis=1)
