...
try:
    ...
    print(
        f"  Photoelectron image stats: min={photoelectron_image.min():.1f}, max={photoelectron_image.max():.1f}, mean={photoelectron_image.mean():.1f}"
    )
    ...
    if len(detected_true) != len(detected_false):
        ...
        print(
            f"   Different spot counts confirm both conditions are working differently"
        )
        print(
            f"   Same image → Different results = Proper bayer_image parameter effect"
        )
    else:
        ...
        if len(detected_true) > 0 and len(detected_false) > 0:
            ...
            pos_diff = (
                np.linalg.norm(detected_true - detected_false, axis=1).mean()
                if len(detected_true) == len(detected_false)
                else float("inf")
            )
            if pos_diff > 1.0:  # More than 1 pixel difference
                print(
                    f"   Position differences found (mean: {pos_diff:.1f} pixels) - algorithms are working differently ✅"
                )
            else:
                print(
                    f"   Very similar positions - algorithms may be giving similar results"
                )

    print(f"\n✅ Same-image comparison test completed!")
    print(
        f"   Both bayer_image=True and bayer_image=False tested on identical photoelectron image"
    )
    ...
    if len(results) >= 2:  # Should have 2 results (1 bootstrap × 2 bayer conditions)
        ...
        for _, row in results.iterrows():
            print(
                f"   Bootstrap {int(row['bootstrap_idx'])}, bayer_image={row['bayer_processing']}: {int(row['n_detected'])} spots detected"
            )
