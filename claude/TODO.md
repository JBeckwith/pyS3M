...
### Priority 2: Plotting Architecture Consolidation

**Status:** 📋 PLANNED
**Analysis:** See Plotting_Architecture_Analysis.md for full details

### Task 1.1: Migrate PlottingFunctions Users to PlottingBase

**Status:** 📋 PLANNED
**Priority:** MEDIUM
**Impact:** ~1,000 line reduction, cleaner architecture
**Effort:** 1-2 weeks
**Risk:** Low (PlottingFunctions already inherits from PlottingBase)

**Problem:**
- PlottingBase.py created during Phase 1 DRY refactoring (modern architecture)
- PlottingFunctions.py is legacy compatibility wrapper (~1,260 lines)
- Both files ~1,250 lines each
- 4 files still use old PlottingFunctions import
- Two entry points for similar functionality causing potential confusion

**Solution:**
Migrate codebase to use PlottingBase directly, deprecate PlottingFunctions wrapper.

**Files to Migrate:**
1. [ ] SR_Functions.py (line 22: `import PlottingFunctions`)
2. [ ] DriftCorrectionFunctions.py (lines 100, 2752, 3289)
3. [ ] DriftPlotting.py
4. [ ] FiducialDetection.py

**Migration Steps:**
1. [ ] Update SR_Functions.py:
   - Change `import PlottingFunctions` → `from PlottingBase import PublicationPlotter`
   - Update `PlottingFunctions.Plotter()` → `PublicationPlotter()`
   - Test all plotting functions
2. [ ] Update DriftCorrectionFunctions.py:
   - Replace lazy imports with PlottingBase
   - Test drift correction plotting
3. [ ] Update DriftPlotting.py and FiducialDetection.py
4. [ ] Run full test suite (all 12+ tests)
5. [ ] Add deprecation warning to PlottingFunctions.Plotter
6. [ ] Update any notebooks/scripts using old import

**Post-Migration Cleanup:**
- [ ] Reduce PlottingFunctions.py to just PlotConstants (~200 lines)
- [ ] Or remove entirely and move PlotConstants to PlottingBase
- [ ] Update documentation to recommend PlottingBase

**Expected Benefits:**
- Single entry point for plotting (PlottingBase)
- Clearer architecture for new contributors
- ~1,000 line reduction
- Easier maintenance going forward

**Notes:**
- PlottingFunctions currently **inherits** from PlottingBase.PublicationPlotter
- No reimplementation, so behavior already unified
- Migration is low-risk refactoring, not functionality change
- New code (like `multichannel_overlay_plot`) already uses PlottingBase ✅
...
**Current issue:** Full frame arrays held in memory during RCC
