...
class InteractiveFiducialSelector:
    ...
    def create_localization_image(self, blur_method: str = "gaussian") -> np.ndarray:
        ...
        try:
            ...
            image = render.render(
                ...
                1
            ]  # Take the rendered image, not the info
    ...
    def _extract_fiducial_coordinates(
        self, locs_with_groups: np.recarray
    ...
    def manual_fiducial_selection(
        self, pick_radius_nm: float = 500.0, min_localizations: int = 50
    ) -> FiducialSelectionResult:
        """
        ...
        pick_radius_pixels = pick_radius_nm / (
            self.pixelsize * 1000
        )  # Convert to pixels
        ...
        for i, (x, y) in enumerate(self.picked_coordinates):
            ...
            if len(within_radius) >= min_localizations:
                ...
                print(
                    f"✅ Fiducial {i+1}: {len(within_radius)} localizations at ({x:.2f}, {y:.2f})"
                )
            else:
                print(
    ...
    def _create_grouped_localizations(
        self, picked_locs_list: List[np.recarray]
    ) -> np.recarray:
    ...
    def preview_fiducials(self, selection_result: FiducialSelectionResult):
        ...
        ax.set_ylabel("Y (μm)")
        ax.set_title(
            f"Selected Fiducials ({selection_result.selection_method})\n"
    ...
    def _save_fiducial_preview(
        self, selection_result: FiducialSelectionResult, filename: str
    ):
...
def perform_fiducial_drift_correction(
    selection_result: FiducialSelectionResult, info: List[dict]
    ...
    print(
        f"Using {selection_result.n_fiducials} fiducials from {selection_result.selection_method} selection"
    )
    ...
    corrected_locs, drift_result = drift_corrector.undrift(
        locs=selection_result.selected_localizations, info=info, method="fiducial"
    )
...
def analyze_fiducial_quality(
    selection_result: FiducialSelectionResult, drift_result: DCF.DriftResult
) -> Dict[str, Any]:
    """
    ...
    print(
        f"  - Average localizations per fiducial: {metrics['mean_localizations_per_fiducial']:.1f}"
    )
    print(
        f"  - Mean fiducial precision: X={metrics['mean_std_x']*1000:.1f} nm, Y={metrics['mean_std_y']*1000:.1f} nm"
    )
    ...
    for i, fid_stat in enumerate(fiducial_stats):
        print(
            f"  Fiducial {fid_stat['group_id']+1}: {fid_stat['n_localizations']} locs, "
...
def plot_fiducial_drift_results(
    ...
    ax1.scatter(
        original_locs.xc, original_locs.yc, s=0.5, alpha=0.3, c="red", label="All locs"
    )
    ...
    # Overall title
    fig.suptitle(
        f"Fiducial Drift Correction Results\n"
...
def main():
    ...
    corrected_locs, drift_result = perform_fiducial_drift_correction(
        selection_result, info
    )
    ...
    plot_fiducial_drift_results(locs, corrected_locs, selection_result, drift_result)
    ...
    print(
        f"✅ Max drift corrected: {quality_metrics.get('max_drift_magnitude', 0):.3f} pixels"
    )
    print(
        f"✅ Mean fiducial precision: {quality_metrics.get('mean_std_x', 0)*1000:.1f} nm (X), {quality_metrics.get('mean_std_y', 0)*1000:.1f} nm (Y)"
    )
