...
# Adapted from batch_analysis.sh for 20250930 Bacteria NR4A with Sina experiment
...
# Uses variance-aware demosaicing and temporal median settings from interactive threshold tuner
...
THRESHOLD_PARAMS_FILE="$SCRIPT_DIR/20250930_nile_red_threshold_parameters.txt"
...
check_threshold_params() {
    if [ ! -f "$THRESHOLD_PARAMS_FILE" ]; then
        echo "ERROR: 20250930_nile_red_threshold_parameters.txt not found!"
        echo "Please run the interactive threshold tuner first:"
        echo "  source /home/jbeckwith/.virtualenvs/pyBayerSMLM/bin/activate"
        echo "  python superres_notebooks/20250930_NileRedAnalysisTuner.py"
        echo ""
        ...
        log_message "ERROR: 20250930_nile_red_threshold_parameters.txt missing - 20250930_NileRedAnalysisTuner must be run first"
        exit 1
    fi

    # Validate file format (check for pipe-delimited content)
    if ! grep -q "|" "$THRESHOLD_PARAMS_FILE"; then
        echo "WARNING: 20250930_nile_red_threshold_parameters.txt format may be invalid (no pipe delimiters found)"
        log_message "WARNING: 20250930_nile_red_threshold_parameters.txt format validation failed - no pipe delimiters"
    fi
...
process_folder() {
    ...
    # Detailed logging
    {
        ...
        echo "EVER mode: $temporal_median_mode (0=NONE, 1=FITTING_ONLY, 2=DETECTION_AND_FITTING)"
        echo "EVER window: $temporal_median_window frames"
        echo "Started: $(date)"
        echo "========================================"
    } >> "$LOG_FILE"
