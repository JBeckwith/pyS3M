...
def test_hdf5_compatibility():
    ...
    try:
        ...
        print(
            f"Frame range: {combined_data['frame'].min()} to {combined_data['frame'].max()}"
        )
...
def test_large_frame_numbers():
    ...
    print(
        f"Large frame numbers: {large_frame_data['frame'].min()} to {large_frame_data['frame'].max()}"
    )
    ...
    try:
        ...
        print(
            f"Read back frame range: {read_data['frame'].min()} to {read_data['frame'].max()}"
        )
        ...
        if read_data["frame"].dtype in ["int32", "int64"]:
            ...
            print(
                f"✗ Frame dtype should be int32/int64, got {read_data['frame'].dtype}"
            )
            return False
