...
def LRtest(datain):
    ...
    for k in range(nd - 1):                                                     #for k=1:(nd-1)
        if (S[k] > 0 and SS - S[k] > 0):                                        # if (S(k) > 0 & SS-S(k) > 0) %this keeps from taking log(0)
            L[k] = (S[k] * np.log(S[k] / (k + 1)) +                             #L(k)=S(k)*log(S(k)/k)+
                   (SS - S[k]) * np.log((SS - S[k]) / (nd - (k + 1))) -         #(SS-S(k))*log((SS-S(k))/(nd-k))-
                   SS * np.log(SS / nd))                                        #SS*log(SS/nd);
...
nCPnt = 2                                                                       #nCPnt=2;
while crntChange < nData - 1:                                                   #while(crntChange<nData) % when not every change points are found
    if (nxtChange - crntChange) > WinSize:                                      #if((nxtChange-crntChange)>WinSize) % if the current data segment is long enough, try to find a change point inside this segment
        datain = data[crntChange+1 : nxtChange+1]                               #datain=data((crntChange+1):nxtChange); 
...
#LM is a threshold. For now you have to try.
        if LM > 400:                                                            #if(LM>400) 
            ...
            if crntChange < nData - 1:                                          #if(crntChange<nData)
                ...
                while nxtChange < nData and ChangePnt[nxtChange] == 0:          #while(ChangePnt(nxtChange)==0)                                      
                    nxtChange += 1                                              #nxtChange=nxtChange+1;
...
for j in range(len(tCPnt)-1):                                                   #for j=1:(nCPnt-1)
    start = tCPnt[j]                                                            #Ilevel(j)=sum(data((tCPnt(j)+1):tCPnt(j+1)))/(tCPnt(j+1)-tCPnt(j));
