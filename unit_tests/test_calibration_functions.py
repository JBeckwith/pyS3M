...
class TestCalibrationFunctions:
    ...
    def test_gain_map_with_variation(self, calibration_functions):
        ...
        variable_gain = calibration_functions.generate_gain_map(
            height, width, method='variable', 
            gain_mean=1.0, gain_std=0.1
        )
    ...
    def test_calibration_map_saving_loading(self, calibration_functions, sample_camera_params, temp_calibration_dir):
        ...
        calibration_functions.save_calibration_map(sample_camera_params['gain'], gain_path)
        calibration_functions.save_calibration_map(sample_camera_params['offset'], offset_path)
        calibration_functions.save_calibration_map(sample_camera_params['variance'], variance_path)
        ...
        np.testing.assert_array_almost_equal(loaded_gain, sample_camera_params['gain'], decimal=5)
        np.testing.assert_array_almost_equal(loaded_offset, sample_camera_params['offset'], decimal=5)
        np.testing.assert_array_almost_equal(loaded_variance, sample_camera_params['variance'], decimal=5)
    ...
    def test_calibration_validation(self, calibration_functions, sample_camera_params):
        ...
        is_valid, errors = calibration_functions.validate_calibration(sample_camera_params)
        assert is_valid
        assert len(errors) == 0
    ...
    def test_calibration_shape_consistency(self, calibration_functions):
        ...
        variance = calibration_functions.generate_variance_map(height, width, readnoise=1.0, gain_map=gain)
    ...
    def test_gain_map_statistical_properties(self, calibration_functions):
        ...
        gain_map = calibration_functions.generate_gain_map(
            height, width, method='gaussian',
            gain_mean=target_mean, gain_std=target_std
        )
    ...
    def test_dark_current_correction(self, calibration_functions, sample_camera_params):
        ...
        assert corrected.mean() == pytest.approx(test_image.mean() - dark_current, abs=0.1)
    
    @pytest.mark.integration
    def test_complete_calibration_workflow(self, calibration_functions, temp_calibration_dir):
        ...
        is_valid, errors = calibration_functions.validate_calibration(calibration_params)
        assert is_valid, f"Calibration validation failed: {errors}"
        
        # Step 4: Save calibration
        calibration_functions.save_complete_calibration(calibration_params, temp_calibration_dir)
        
        # Step 5: Load and verify
        loaded_params = calibration_functions.load_complete_calibration(temp_calibration_dir)
        
        # Verify loaded parameters match
        np.testing.assert_array_almost_equal(loaded_params['gain'], calibration_params['gain'])
        np.testing.assert_array_almost_equal(loaded_params['offset'], calibration_params['offset'])
        np.testing.assert_array_almost_equal(loaded_params['variance'], calibration_params['variance'])
...
class TestAdvancedCalibrationFeatures:
    ...
    def test_linearity_correction(self, calibration_functions):
        ...
        nonlinear_response = input_levels + 0.0001 * input_levels**2  # Small nonlinearity
    ...
    def test_pixel_response_uniformity(self, calibration_functions):
        ...
        response_map = 1.0 - 0.3 * (distance_from_center / np.max(distance_from_center))**2
        
        # Apply uniformity correction
        correction_map = calibration_functions.calculate_uniformity_correction(response_map)
    ...
    def test_multi_point_calibration(self, calibration_functions):
        ...
        measured_levels = [98, 502, 1005, 1995, 4010]   # Measured ADU (with small errors)
    ...
    @pytest.mark.integration
    def test_calibration_quality_assessment(self, calibration_functions, sample_camera_params):
        """Test calibration quality assessment metrics."""
        # Assess calibration quality
        quality_metrics = calibration_functions.assess_calibration_quality(sample_camera_params)
        ...
        for metric in expected_metrics:
            if metric in quality_metrics:
                ...
                assert quality_metrics[metric] >= 0  # Quality metrics should be non-negative
