...
class TestSpectralConstants:
    ...
    def test_wavelength_config(self):
        ...
        assert SpectralConstants.WAVELENGTH_RANGE[0] < SpectralConstants.WAVELENGTH_RANGE[1]
...
class TestDyeSpectrumProcessor:
    ...
    def mock_handler(self):
        ...
        handler.query_dye_data.return_value = pd.DataFrame({
            ...
            'extinction': np.random.random(30)
        })
        return handler
    ...
    def test_process_spectrum_data(self, processor, mock_handler):
        ...
        with patch.object(processor, '_get_database_handler', return_value=mock_handler):
...
class TestSpectralFunctions:
    ...
    def test_get_dye_data_integration(self, spectral_functions):
        ...
        with patch.object(spectral_functions._dye_processor, 'process_spectrum', return_value=mock_data):
    ...
    def test_get_filter_data_integration(self, spectral_functions):
        ...
        mock_data = pd.DataFrame({
            'wavelength': np.arange(450, 650, 5),
            'transmission': np.random.random(40)
        })
        
        with patch.object(spectral_functions._filter_processor, 'process_spectrum', return_value=mock_data):
    ...
    def test_backward_compatibility_method(self, spectral_functions):
        ...
        with patch.object(spectral_functions, 'get_filter_data', return_value=mock_data):
...
class TestSpectralProcessingWorkflow:
    ...
    def test_complete_dye_workflow(self, spectral_functions):
        ...
        mock_raw_data = pd.DataFrame({
            ...
            'extinction_coeff': np.exp(-((np.arange(400, 700, 5) - 488)**2) / (2 * 30**2))
        })
        
        with patch.object(DatabaseQueryHandler, 'query_dye_data', return_value=mock_raw_data):
            result = spectral_functions.get_dye_data('ATTO488')
    ...
    def test_complete_filter_workflow(self, spectral_functions):
        ...
        with patch.object(DatabaseQueryHandler, 'query_filter_data', return_value=mock_raw_data):
            result = spectral_functions.get_filter_data('BP525/50')
    ...
    def test_error_handling_workflow(self, spectral_functions):
        ...
        with patch.object(DatabaseQueryHandler, 'query_dye_data', side_effect=Exception("Database error")):
            with pytest.raises(Exception, match="Database error"):
                spectral_functions.get_dye_data('NonexistentDye')
        
        # Test empty result handling
        with patch.object(DatabaseQueryHandler, 'query_filter_data', return_value=pd.DataFrame()):
    ...
    def test_performance_with_large_datasets(self, spectral_functions):
        ...
        with patch.object(DatabaseQueryHandler, 'query_dye_data', return_value=large_data):
