...
class TestHelperFunctions:
    ...
    @pytest.mark.unit
    def test_clean_database_basic_functionality(self, helper_functions, sample_database):
    ...
    def test_clean_database_numeric_conversion(self, helper_functions):
        ...
        np.testing.assert_array_almost_equal(
            cleaned_db['numeric_str1'].to_numpy(), 
            expected_values1
        )
        np.testing.assert_array_almost_equal(
            cleaned_db['numeric_str2'].to_numpy(), 
            expected_values2
        )
    ...
    def test_file_search_nonexistent_directory(self, helper_functions):
        ...
        files = helper_functions.file_search('/nonexistent/directory', 'string1', 'string2')
        assert len(files) == 0
        assert isinstance(files, np.ndarray)
    ...
    def test_database_cleaning_workflow(self, helper_functions):
        ...
        test_db = pl.DataFrame(data)
        columns = ['x_position', 'y_position', 'intensity', 'frame_number', 'analysis_file']
        ...
        # Check values
        np.testing.assert_array_almost_equal(
            cleaned_db['x_position'].to_numpy(),
            [12.34, 56.78, 90.12]
        )
        np.testing.assert_array_almost_equal(
            cleaned_db['intensity'].to_numpy(),
