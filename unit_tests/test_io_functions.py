...
class TestIOFunctions:
    ...
    def test_add_photon_columns(self, io_functions, sample_dataframe):
        ...
        df_with_photons = io_functions._add_photon_columns(sample_dataframe.copy(), normalize=False)
        ...
        expected_photons = sample_dataframe['A_B'] + sample_dataframe['A_G'] + sample_dataframe['A_R']
        expected_bg_photons = sample_dataframe['bg_B'] + sample_dataframe['bg_G'] + sample_dataframe['bg_R']
        
        np.testing.assert_array_equal(df_with_photons['photons'], expected_photons)
        np.testing.assert_array_equal(df_with_photons['background_photons'], expected_bg_photons)
    ...
    def test_tiff_write_read_workflow(self, io_functions):
        ...
        try:
            ...
            # Write TIFF
            io_functions.write_tiff(original_stack, temp_filename, bit="uint16", pixel_size=0.1)
