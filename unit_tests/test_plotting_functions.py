...
class TestPlottingIntegration:
    ...
    def complex_dataset(self):
        ...
        return {
            ...
            'signal1': np.sin(2 * np.pi * np.linspace(0, 10, n_points)) + 0.1 * np.random.randn(n_points),
            'signal2': np.cos(2 * np.pi * np.linspace(0, 10, n_points)) + 0.1 * np.random.randn(n_points),
    ...
    def test_multi_panel_figure(self, plotting_functions, complex_dataset):
        ...
        axes[1, 2].boxplot([complex_dataset['values'][complex_dataset['categories'] == cat] 
                           for cat in ['A', 'B', 'C']])
    ...
    @pytest.mark.integration
    def test_publication_quality_figure(self, plotting_functions, complex_dataset, temp_plot_file):
        ...
        ax.plot(complex_dataset['time'], complex_dataset['signal1'], 
                linewidth=1.0, color='black', label='Experimental')
        ax.plot(complex_dataset['time'], complex_dataset['signal2'],
                linewidth=1.0, color='red', linestyle='--', label='Control')
        ...
        plotting_functions.save_plot(fig, temp_plot_file.replace('.png', '_pub.png'), dpi=300)
        
        plt.close(fig)
    
    @pytest.mark.integration  
    def test_interactive_plotting_compatibility(self, plotting_functions, complex_dataset):
