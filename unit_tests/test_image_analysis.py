...
class TestFittingStrategy:
    ...
    def test_strategy_enum_completeness(self):
        ...
        expected_values = {"standard", "nocolour", "justcolour", "rawcolour", "posthencolour"}
...
class TestFittingParameters:
    ...
    def test_inconsistent_array_lengths(self, sample_puncta_data):
        ...
        sample_puncta_data['smoothed_puncta'] = sample_puncta_data['smoothed_puncta'][:-1]
...
class TestFittingResultProcessor:
    ...
    def test_calculate_errors_none_covariance(self):
        ...
        expected_size = FittingConstants.PARAM_DIMENSIONS[FittingStrategy.STANDARD]["error"]
    ...
    def test_calculate_errors_infinite_covariance(self):
        ...
        expected_size = FittingConstants.PARAM_DIMENSIONS[FittingStrategy.NOCOLOUR]["error"]
        assert len(errors) == expected_size
        assert all(np.isnan(errors))
    ...
    def test_process_fit_results_standard_strategy(self):
        ...
        np.testing.assert_allclose(pfit_final[4:7], [4.0, 9.0, 6.25])  # squared backgrounds
        np.testing.assert_allclose(pfit_final[7:10], [100.0, 225.0, 144.0])  # squared amplitudes
        
        # Check chi-squared was appended
        assert pfit_final[-1] == chisqr
        
        # Check errors were calculated
        assert len(errors) == FittingConstants.PARAM_DIMENSIONS[FittingStrategy.STANDARD]["error"]
...
class TestSyntheticDataGeneration:
    ...
    def test_synthetic_gaussian_generation(self):
        ...
        image, ground_truth = self.generate_synthetic_gaussian_punctum(
            size=11, amplitude=1000.0, background=100.0,
            x_center=5.0, y_center=5.0, sigma=1.2,
            random_seed=42
        )
...
class TestImageAnalysisFunctionsBasic:
    ...
    def test_available_strategies(self):
        ...
        expected_names = ["standard", "nocolour", "justcolour", "rawcolour", "posthencolour"]
